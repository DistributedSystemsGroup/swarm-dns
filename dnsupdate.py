...
class DDNSUpdater:
    ...
    :type algo: str
    """
    def __init__(self, server, keyfile, domain, ttl=DEFAULT_TTL):
        self.server = server
    ...
    def _read_key(self):
        try:
            ...
            log.error("DDNS key not found, cannot update DNS")
            return
        keyfile = f.read()
        f.close()
        ...
        try:
            ...
            log.error('{} is not a valid key file. The file should be in DNS KEY record format. See dnssec-keygen(8)'.format(self.keyfile))
